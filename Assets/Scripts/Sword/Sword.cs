            ...
            if (Trigger.Collided)
            {
                ...
                if (@object.tag == "Player" && !@object.GetComponent<PhotonView>().IsMine)
                    if (!mCollideded.Contains(@object))
                    {
                        ...
                        if (player.OwnSword.mIsGuard)
                        {
                            float angle = Quaternion.FromToRotation(Vector3.right, @object.transform.position - transform.position).eulerAngles.z + 180f;

                            float attack = player.OwnSword.mCurrentAngle - angle;

                            if (attack > 180f) attack -= 360f;
                            if (attack < 180f) attack += 360f;

                            Debug.Log($"{player.OwnSword.mCurrentAngle}, {angle}, {attack}");

                            if (Mathf.Abs(attack) <= 60f) return;
                        }
                        player.Damage(12);
                    }
            }
